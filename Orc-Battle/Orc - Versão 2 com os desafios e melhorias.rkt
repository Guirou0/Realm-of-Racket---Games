...
(struct berserker monster () #:transparent)
...
(define BERSERKER-IMAGE (bitmap "graphics/berserker.png"))
...
(define INSTRUCTION-TEXT (text "S: Facada| F: Ataque em área| H: Curar (Custa um ataque)\nD: Descansar (Recupera força e reduz um ataque)| R: Respirar (Recupera agilidade e reduz um ataque)\nB: Bloquear (Aumenta a armadura e reduz um ataque)" 10 "black"))
(define V-SPACER (rectangle 0 10 "solid" "white"))
(define H-SPACER (rectangle 10 0 "solid" "white"))
(define TARGET (bitmap "graphics/target.png"))
  ...
  (build-list
   ...
   (lambda (_)
     ...
     (case (random 5)
       ...
       [(4) (berserker BERSERKER-IMAGE health)]))))

(define (instructions w)
  (define na (number->string (orc-world-attack# w)))
  (define ra (string-append "Ataques restantes: " na))
...
(define (endure w)
  (decrease-attack# w)
  (player-strength+ (orc-world-player w) ENDURE))

(define (agile w)
  (decrease-attack# w)
  (player-agility+ (orc-world-player w) AGILE))
...
(define (all-monsters-attack-player player lom)
  (define armory (if (zero? (player-armor player)) 0 (random+ (player-armor player))))
  (player-armor+ player (- 0 armory))
  (define (one-monster-attacks-player m)
    (cond
      [(orc? m)
       (player-health+ player (armor-damage armory (random- (orc-club m))))]
      [(hydra? m)
       (player-health+ player (armor-damage armory (random- (monster-health m))))]
      [(slime? m)
       (player-health+ player (armor-damage armory -1))
       (player-agility+ player
                        (random- (slime-sliminess m)))]
      [(brigand? m)
       (case (random 3)
         [(0) (player-health+ player (armor-damage armory HEALTH-DAMAGE))]
...
(define (armor-damage armor dmg)
  (if (> (+ armor dmg) 0) 0 (+ armor dmg)))
