...
(define (render-entree entree)
  ...
  (add-path-hack id pl (add-players id pl (add-food fd BASE))))

;add-path-hack não funciona: medium challenge ok
...
(define (handle-entree-message s msg)
  ;(println (list (entree-id s) "......" (player-waypoints (first (entree-players s))) (player-waypoints (second (entree-players s)))))
