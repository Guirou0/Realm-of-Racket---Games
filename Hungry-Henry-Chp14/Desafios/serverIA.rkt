...
;VERSÃO DO server.rkt QUE POSSUI UM PLAYER IA
;Feito para tentar completar o difficult challenge do livro
...
(define BASE-SPEED 400)
(define LIMIT-SIZE 200)
...
(define IA (player "IA" (create-a-body PLAYER-SIZE) '()))
...
(define (broadcast-universe p)
  (define mails (broadcast-safe (get-iws p) p))
  (make-bundle p mails empty))

(define (broadcast-safe iws p)
  (define players (play-players p))
  (define food (play-food p))
  (for/list ([iw iws])
    (safe-mail iw players food)))

(define (safe-mail iw players food)
  (define sf (append (safe-players iw players) (list (player "IA" (player-body IA) '()))))
  (make-mail iw (list SERIALIZE sf food)))

(define (safe-players iw players)
  (for/list ([p players])
    (define pl (ip-player p))
    (cond [(iworld=? iw (ip-iw p))
           pl]
          [else (player (player-id pl) (player-body pl) empty)])))

(define (get-iws p)
  (define clients (append (play-players p) (play-spectators p)))
  (for/list ([c clients])
    (ip-iw c)))
...
(define (move-and-eat pu)
  (set! IA (update-ia IA (play-food pu)))
...
(define (update-ia ia fd)
  ;(println ia)
  (if (empty? (player-waypoints ia))
      (ia-waypoint ia fd)
      (player "IA" (player-body ia) (move-toward-waypoint (player-body ia) (player-waypoints ia)))))

(define (ia-waypoint ia fd)
  (define waypoint (close-waypoint ia fd))
  ;(println waypoint)
  (player "IA" (player-body ia) (list waypoint)))

(define (close-waypoint ia fd)
  (define ia-loc (body-loc (player-body ia)))
  (define dists (map (lambda (f) (cdist (body-loc f) ia-loc)) fd))
  (define mindist (foldr min (first dists) (rest dists)))
  (define mindex (index mindist dists))
  (body-loc (list-ref fd mindex)))
...
(define (feed-em-all players foods)
  (define newfoods (ia-eat foods))
  (for/fold ([food newfoods]) ([p players])
    (eat-all-the-things p food)))

(define (ia-eat foods)
  (define b (player-body IA))
...
;eat-all-the-things modificado: easy challenge ok
...
(define (progress pls foods spectators)
  (define sz (+ (body-size (player-body IA)) (sum-size pls))) ;soma dos pesos de cada player
  (define p (play pls foods spectators))
  ;(println sz)
  ;Modificação para que o jogo não se torne infinito. Agora, o jogo acaba quando a soma dos pesos dos players atinge um limite
  (cond [(>= sz LIMIT-SIZE) (end-game-broadcast p)]
        [else (broadcast-universe p)]))

(define (sum-size pls)
  (for/fold ([total 0]) ([pl pls])
    (+ total (body-size (player-body (ip-player pl))))))

(define (end-game-broadcast p)
  (define iws (get-iws p))
  (define msg (list SCORE (cons (list "IA" (get-score (body-size (player-body IA)))) (score (play-players p)))))
...
(define (remake-join p)
  (set! IA (player "IA" (create-a-body PLAYER-SIZE) '()))
  (define players (refresh (play-players p)))
  (define spectators (play-spectators p))
  (join (append players spectators) 0))
